...
int main(int argc, char ** argv) {
	...
	*/
	...
	for (t=0;t<T;t++) {
		...
		if (north >= 0) {
			MPI_Irecv(&u_current[0][0], local[1] + 2, MPI_DOUBLE, north, t, MPI_COMM_WORLD, &requests[req_length]);
			req_length++;
		}
		if (west >= 0) {
		...
		if (north >= 0) {
			MPI_Isend(&u_current[1][0], local[1] + 2, MPI_DOUBLE, north, t, MPI_COMM_WORLD, &requests[req_length]);
			req_length++;
		}		
		if (south >= 0) {
			MPI_Isend(&u_current[local[0]][0], local[1] + 2, MPI_DOUBLE, south, t, MPI_COMM_WORLD, &requests[req_length]);
			req_length++;
			MPI_Irecv(&u_current[local[0]+1][0], local[1] + 2, MPI_DOUBLE, south, t, MPI_COMM_WORLD, &requests[req_length]);
			req_length++;
		}
		if (east >= 0) {
			MPI_Isend(&u_current[0][local[1]], 1, column, east, t, MPI_COMM_WORLD, &requests[req_length]);
			req_length++;
			MPI_Irecv(&u_current[0][local[1]+1], 1, column, east, t, MPI_COMM_WORLD, &requests[req_length]);
			req_length++;
		}
		if (west >= 0) {
			MPI_Isend(&u_current[0][1], 1, column, west, t, MPI_COMM_WORLD, &requests[req_length]);
			req_length++;
		}
		MPI_Waitall(req_length, requests, statuses);
